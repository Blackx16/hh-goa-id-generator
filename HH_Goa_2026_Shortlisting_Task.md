...
1\. User uploads a photo (support common formats \- JPG, PNG, HEIC from iPhone). 2\. (Format B only) User fills in a couple of quick fields: name, stack/role, etc. 3\. Tool generates the final graphic; should feel near-instant. 
...
• Speed: Upload to finished result should be fast—a few seconds, not a loading screen. • Handles real photos: Portrait, landscape, off-center crops, different aspect ratios; don't assume users will crop first. 

• On-brand: It should be unmistakably this event, not a generic badge with a logo pasted on. • Downloadable output: A real image file, not something that only renders on-screen. • Working share flow: Pre-filled caption \+ hashtag \#FrameInGoa. If you share via link rather than direct image attach, ensure the link preview (OG image) shows the generated graphic, not a blank/default thumbnail. 
...
⚠ Remember: Submissions without an X post containing \#FrameInGoa will be treated as invalid. Where to submit   
